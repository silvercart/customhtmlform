<div id="{$FormName}_{$FieldName}_Box" class="control-group<% if errorMessage %> error<% end_if %>">
    ...
    <div class="input-prepend row-fluid">
        <span class="add-on"><label class="control-label" for="{$FieldID}"><i class="icon-lock"></i></label></span>
        <input class="span6" type="password" placeholder="{$Label}" name="{$FieldName}" id="{$FieldID}">
    </div>
</div>